...
class Service(BaseService):
    ...
    def create_account(
        ...
        # Typically no authorization needed to create an account, or done in route
    ...
    def get_account(self, actor: SupportsPermissionCheck, account_id: UUID) -> ServiceResult[Account]:
        self._authorize(actor, "accounts", "account", "read")
        with self._uow_factory() as uow:
    ...
    def list_accounts(self, actor: SupportsPermissionCheck, filters: AccountFilter) -> ServiceResult[Pagination[Account]]:
        self._authorize(actor, "accounts", "account", "list")
        with self._uow_factory() as uow:
    ...
    def update_account(
        ...
        self._authorize(actor, "accounts", "account", "update")
        with self._uow_factory() as uow:
            account = uow.accounts.get(account_id)
            if not account:
                raise AccountNotFound()
    ...
    def suspend_account(self, actor: SupportsPermissionCheck, account_id: UUID) -> ServiceResult[Account]:
        self._authorize(actor, "accounts", "account", "suspend")
        with self._uow_factory() as uow:
            account = uow.accounts.get(account_id)
            if not account:
                raise AccountNotFound()
