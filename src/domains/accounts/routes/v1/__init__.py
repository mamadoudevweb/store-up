...
def list_accounts():
    ...
    result = _get_domain_service().accounts.account.list_accounts(filters)
...
def get_account(account_id: UUID):
    result = _get_domain_service().accounts.account.get_account(account_id)
...
def suspend_account(account_id: UUID):
    ...
    result = _get_domain_service().accounts.account.suspend_account(account_id)
    return ok(_serialize_account(result.data))
...
def get_credentials(account_id: UUID):
    result = _get_domain_service().accounts.credential.get_credentials(account_id)
...
def assign_role(account_id: UUID):
    ...
    result = _get_domain_service().accounts.role.assign_role(
        account_id=account_id,
...
def list_account_roles(account_id: UUID):
    result = _get_domain_service().accounts.role.list_roles(account_id)
    data = [
        AccountRoleResponse(
...
def revoke_role(account_id: UUID, role_id: UUID):
    _get_domain_service().accounts.role.revoke_role(account_id, role_id)
    return ok(None)
