...
def get_account_service() -> Any:
    return current_app.extensions["account_service"]


def serialize_account(account) -> dict[str, Any]:  # type: ignore[no-untyped-def]
    return AccountResponse.model_validate(account).model_dump(mode="json")


def serialize_credential(cred) -> dict[str, Any]:  # type: ignore[no-untyped-def]
    return CredentialResponse.model_validate(cred).model_dump(mode="json")


def serialize_account_role(role) -> dict[str, Any]:  # type: ignore[no-untyped-def]
