...
class StockItemModel(db.Model):  # type: ignore[name-defined]
    __tablename__ = "stock_items"

    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    variant_id: Mapped[uuid.UUID] = mapped_column(unique=True, nullable=False, index=True)
...
class StockMovementModel(db.Model):  # type: ignore[name-defined]
    ...
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    stock_item_id: Mapped[uuid.UUID] = mapped_column(nullable=False, index=True)
    ...
    __table_args__ = (
        CheckConstraint(
            "reason IN ('restock', 'order_shipped', 'manual_adjustment', 'return', 'damage')", 
            name="chk_stock_movement_reason"
        ),
        CheckConstraint(
            "reference_type IN ('sale', 'purchase_order', 'manual_adjustment', 'return')", 
            name="chk_stock_movement_ref_type"
        ),
    )
